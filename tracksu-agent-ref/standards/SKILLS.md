...
- Repo rules TSD действуют при чтении TSD: работа там read-only, без ignored
  ...
  правила и конкретное поручение пользователя; сейчас git-изменения не выполняем.
...
и unrelated bugs записать отдельно; следующую feature без выбора не начинать.
