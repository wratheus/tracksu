...
Исходный код: 5431e9a, чистое дерево. Сейчас поиск доступен только после входа;
...
Код: `2c3c566 feat(profile): complete guest-first player exploration`.
Итоговый scoped analyze чистый, повторная debug APK сборка после review: exit 0.
Документация синхронизируется следующим коммитом без изменения кода.
Дальше — ручные сценарии из раздела проверок; рекомендуемый следующий
существенный срез в ROADMAP: P10 (scores и карты профиля), далее P12 (beatmap).
Уточнение пользователя 2026-09-05: темы сначала отдельно планируем и согласуем
с assets/osu!-стилем; они не блокируют восстановление остальных страниц.
