...
| [P08](DETAILS.md#p08) | Auth/session и миграция secure storage | TokenStore, session restore, bearer, code exchange, persistent callback state, refresh, one-shot 401 retry, local logout и `SessionStatus` готовы; legacy migration и ручный flow остаются | in_progress |
