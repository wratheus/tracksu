...
полные локализованные страны, более квадратные аватары и парные флаги 30×20.
Gate — только format/analyze/review; ручная проверка сети, звука и вёрстки открыта.
Данные страниц сохраняют прежний memory snapshot + revalidation контракт.

[P24: команды, рейтинг и настройки](work/P24-teams-and-ranking-density.md).
Нативная Team-страница, плотные ranking rows с PP справа и независимый доступ
к настройкам реализованы; ожидают ручной проверки.
...
и отдельная лицензия/выбор шрифта. Disk media cache отдельно поручен 22 сентября
и реализован в P28. Фоновый плеер и новые backend-фичи сюда не входят.
...
реализованы. [Карточки и графики P10/P12](work/P10-cards-and-charts.md) реализованы:
результаты с modal-подробностями, metadata карт/выбор сложности, line rank и
помесячные replay views. Analyze/main/catalog debug прошли; ожидают device-проверки.
...
Решение от 9 сентября (P18, одноразовое разрешение) заменено поручением
22 сентября: картинки загружаются по умолчанию, отключение в настройках;
ранее явно сохранённый отказ остаётся в силе. Реализация и disclosure — P28.
Ограничения совместимости и limits описаны в контракте; device-проверка ожидается.
[Расширенные результаты и описание карты](work/P10-result-content.md) реализованы:
полученные hit statistics/maximum statistics, типизированные настройки модов и
общий ContentFrame для описания набора/профиля. Ожидают ручной проверки.
[Продуктовая доработка рейтингов](work/P11-product.md) реализована: независимые
mode/sort, аватары, позиции таблицы, метрика по сортировке, country picker с
поиском/флагами и сохранение scroll. Ожидает ручной проверки.
Сейчас — [обратная связь 9 сентября](work/P18-september-feedback.md): баннеры,
интервалы, числа, monthly playcounts, разрешение изображений и Share реализованы,
ожидают ручной проверки. Прежние имена, команды/группы, ranked play и статистика
карты дня подключены и подняты над статистикой. Медали вынесены в страницу
с именами/описаниями/картинками из public web bootstrap osu! (нестабильный
контракт, обработка ошибок обязательна). Настройки — отдельная страница,
модалки content-fit/draggable, детали карты и результата переработаны.
