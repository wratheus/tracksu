...
По дополнению пользователя 2026-09-08 повторный tap текущей вкладки возвращает
её в root через goBranch(initialLocation: true). На уже открытом root не
сбрасываем фильтры/scroll и не запускаем refresh. Стек деталей этой ветки при
таком явном reselect закрывается; другие ветки остаются сохранёнными.
Это заменяет прежнее правило no-op reselect. Фокус при выборе вкладки снимается.
Нижняя панель остаётся на профиле/карте/статье, OAuth скрывает её.
...
- Retap активной вкладки с деталей → её root; retap на root сохраняет
  фильтры/scroll. Переход к другой вкладке сохраняет её прежние детали.
- Popup/sheet/клавиатура, крупный шрифт и длинные локализации.
